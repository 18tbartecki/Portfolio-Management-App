...
var originalNotSpecifiedArray = new Array();
...
//			}
			...
			for(let i=0; i < portfolioElem.length; i++) {
				...
				userTable.innerHTML +=
					...
					'<th><button type="button" id="delete-button-stock-test'+i+'" onclick="passInIndex('+i+')" class="btn btn-danger" data-toggle="modal" data-target="#confirmStockDelete"><i class="fas fa-trash-alt"></i> Delete</button>' +
					'</tr>';
			}
...
//	console.log(currentStock);
	...
	request.onreadystatechange = function() {
		if(request.readyState === XMLHttpRequest.DONE && request.status === 200) {
			...
			addViewStock(currTicker, stockDataAPI); // now add to graph
...
//		            },
	...
	// This function adds extra lines to the graph from "View A Stock" input
	...
	// This function removes extra line from the graph
	...
	window.zoomIn = function() {
	    ...
	    */
	    ...
	    //document.getElementById("zoomTests").innerHTML = "zoom in";
	...
	$("#spy-line-toggle").click(function() {
...
function loadPerformaceData() {
	...
	// more than a single stock in portfolio
		...
		if(cagrPortfolio > 0) {
			ARROW1.innerHTML = '<span style="color:green"><i class="fas fa-arrow-alt-circle-up fa-2x"></i></span>';
		}
		
		// if negative, use red arrow
		else {
			ARROW1.innerHTML = '<span style="color:red"><i class="fas fa-arrow-alt-circle-down fa-2x"></i></span>';
		}
		...
		// if positive, use green arrow
		if(cagrIndex > 0) {
			ARROW2.innerHTML = '<span style="color:green"><i class="fas fa-arrow-alt-circle-up fa-2x"></i></span>';
			// <i class="fas fa-arrow-alt-circle-up"></i>
		}
		
		// if negative, use red arrow
		else {
			ARROW2.innerHTML = '<span style="color:red"><i class="fas fa-arrow-alt-circle-down fa-2x"></i></span>';
		}
	}
}
